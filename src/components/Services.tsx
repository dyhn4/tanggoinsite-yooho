...
export default function Services() {
  return (
    <section id="services" className="py-24 px-4 sm:px-6 bg-[#f8f9ff]">
      <div className="max-w-6xl mx-auto">
        {/* 헤더 — tango.ai.kr .section-header 스타일 */}
        ...
        {/* 서비스 그리드 — tango.ai.kr .feature-card 스타일 */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((service) => (
            <div
              key={service.title}
              className="group bg-white rounded-3xl p-7 border border-[#e8ecf4] hover:-translate-y-2.5 hover:shadow-2xl hover:border-violet-200 transition-all duration-300 cursor-default"
              style={{ boxShadow: "0 2px 10px rgba(108, 92, 231, 0.07)" }}
            >
              {/* 아이콘 박스 — tango.ai.kr .feature-icon 스타일 */}
              <div
                className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-5 bg-gradient-to-br ${service.gradient} shadow-lg ${service.shadow}`}
              >
                <service.icon size={24} className="text-white" />
              </div>

              <h3 className="font-bold text-[#2d3561] text-lg mb-3 group-hover:text-violet-700 transition-colors">
                {service.title}
              </h3>
              <p className="text-[#7e8ba3] text-sm leading-relaxed mb-4">
                {service.description}
              </p>
