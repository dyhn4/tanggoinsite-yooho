...
const solutions: AnySolution[] = [
  {
    ...
    processes: [
      { icon: Briefcase,      label: "사업관리",  items: ["사업 등록/관리 및 장서 등록/관리", "일정·작업 실적 모니터링", "공정별 작업 배분 및 권한 관리", "작업자/점검자 ID/PW 로그인"] },
      { icon: ShieldCheck,    label: "품질관리",  items: ["전체 공정 통합 계획 수립", "검증된 노하우 기반 품질관리", "공정별 품질 지표 실시간 집계"] },
      ...
      { icon: Settings,       label: "구축관리",  items: ["작업확정 목록 등록", "공정별 작업 상세 내역 등록", "작업이력 조회·점검이관·공정별 트래킹"] },
    ],
    automation: [
  ...
  },
  {
    ...
    features: [
      ...
      { icon: "🎯", name: "군집검수",  stage: "Full Text 교정/교열",   desc: "같은 모양으로 인식한 글자들을 모아 한 번에 수정. 에디터 상에서 '단일이미지'로 표현" },
      { icon: "✏️", name: "낱자정정",  stage: "Full Text 교정/교열",   desc: "개별 글자의 텍스트를 선택·수정. 낱자별 교정으로 학습된 인식 오류를 정밀 처리" },
  ...
  },
  {
    ...
    outputs: [
      ...
      { icon: "🖼️", name: "표·이미지, 글자 이미지 추출", desc: "문서 내 표·그림·글자 이미지를 페이지별로 분리하여 독립 파일 저장" },
      { icon: "📑", name: "Hidden Text PDF 생성",       desc: "원본 이미지 위에 텍스트 레이어를 삽입한 검색 가능한 PDF 자동 생성" },
      { icon: "📚", name: "서지 정보 목록 추출",         desc: "제목·저자·발행연도 등 서지 메타데이터를 자동 추출하여 목록 파일 생성" },
  ...
  },
  {
    ...
    features: [
      ...
      { icon: "🖼️", name: "글자 이미지·표·이미지 추출",  desc: "(기사단위) 글자 이미지, 표, 이미지를 각각 분리하여 개별 파일로 저장" },
      { icon: "📂", name: "JPG/TXT/JSON/XML/PDF 분리",  desc: "필요한 형식으로 즉시 변환, 멀티포맷 동시 출력 지원" },
      { icon: "⚖️", name: "면 단위 저작권 상태별 분리",  desc: "저작권 상태(보호 기간·공공 영역 등)에 따라 면 단위 자동 분류 처리" },
    ],
  },
  {
    kind: "explorer",
    ...
    capabilities: [
      ...
      { icon: "🇰🇷", name: "현대 한국어 번역",   desc: "고문헌·근대 문서의 내용을 현대 한국어로 자동 번역하여 접근성 극대화" },
