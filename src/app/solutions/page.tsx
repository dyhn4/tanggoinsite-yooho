...
const solutions = [
  {
    ...
    description:
      "MooN Editor, AI OCR, Tango Batch를 자동으로 연계하는 통합 워크플로우 시스템. 사업관리부터 납품까지 전 공정을 일원화하여 품질과 효율을 동시에 잡습니다.",
  ...
  },
  {
    ...
    badge: "Plug in Program",
    description:
      "MooN AI OCR로 인식된 원문 데이터를 정밀 편집하는 전문 에디터. 군집검수·날자정정·문단정제로 최고 수준의 텍스트 품질을 실현합니다.",
    ...
    features: [
      {
        icon: "🔍", name: "AI OCR",
        desc: "학습 데이터를 토대로 이미지상의 글자를 자동으로 인식하여 입력",
        stage: "AI OCR 입력",
      },
      {
        icon: "🎯", name: "군집검수",
        desc: "같은 모양으로 인식한 글자들을 모아 한번에 수정. 에디터 상에서 '딜일이미지'로 표현",
        stage: "Full Text 교정/교열",
      },
      {
        icon: "✏️", name: "날자정정",
        desc: "개별 글자의 텍스트를 선택·수정. 날자별 교정으로 학습된 인식 오류를 정밀 처리",
        stage: "Full Text 교정/교열",
      },
      {
        icon: "📝", name: "문단정제",
        desc: "가로쓰기·세로쓰기 혼용 및 1~4단 고문헌에 최적화된 문단 구축·정제",
        stage: "문단 정제",
      },
    ],
  },
  {
    ...
    badge: "Plug in Program",
    description:
      "탱고인사이트가 자체 개발한 Data-Adaptive OCR 엔진. 국·漢文 혼용, 필기체, 고문헌 등 어떠한 문서도 스스로 학습하며 인식률을 높입니다.",
  ...
  },
  {
    ...
    badge: "Plug in Program",
    description:
      "문서를 표준 XML 구조화 파일로 변환하고 의미 기반 JSON으로 자동 출력합니다. 자동 XML Tagging으로 수작업 태깅 시간을 획기적으로 절감합니다.",
    ...
    features: [
      {
        icon: "📄", name: "XML 구조화 파일 변환",
        desc: "원본 문서를 DTD/XSD 기반 XML 구조로 자동 변환하여 표준화된 데이터 형식 제공",
      },
      {
        icon: "🏷️", name: "자동 XML Tagging",
        desc: "문서 내 요소를 AI로 자동 분류하고 XML 태그를 지정 — 수작업 태깅 시간 90% 절감",
      },
      {
        icon: "⚡", name: "의미기반 JSON 파일 변환",
        desc: "문서의 의미 구조를 분석하여 JSON으로 변환, API 연계 및 DB 적재 즉시 가능",
      },
    ],
  },
  {
    ...
    badge: "Plug in Program",
    description:
      "수십만 페이지를 한번에 처리하는 고성능 배치 엔진. XML·JSON·TXT 생성부터 이미지 추출, Hidden Text PDF까지 원스톱으로 완성합니다.",
  ...
  },
  {
    ...
    icon: Newspaper,
    title: "Tango Articlo",
    subtitle: "지능형 기사 단위 데이터 분리",
    badge: "Plug in Program",
    description:
      "신문·잡지·연속간행물을 기사 단위로 자동 분리합니다. 면 단위 저작권 상태별 분리까지 지원하여 디지털 아카이브 구축을 완성합니다.",
  ...
  },
  {
    ...
    subtitle: "AI 기반 지식 탐색·번역 플랫폼",
    badge: "Plug in Program",
    description:
      "구축된 디지털 아카이브 데이터를 AI로 탐색하고 번역합니다. 한국어·영어 번역부터 특정 주제 질의응답까지, 데이터의 가치를 극대화합니다.",
...
export default function SolutionsPage() {
  ...
  return (
    <div className="bg-slate-950 min-h-screen">
      ...
      {/* ── 상단 헤더 ── */}
      <div className="sticky top-0 z-50 border-b border-white/8" style={{ background: "rgba(10,12,26,0.92)", backdropFilter: "blur(16px)" }}>
              ...
              <div className="relative z-10 max-w-7xl mx-auto px-6 sm:px-10 py-20 w-full">
                <div className="flex flex-col md:flex-row md:items-center gap-10">
                  ...
                  <div className="flex-1 animate-fade-up">
                    ...
                    <p className="text-lg font-semibold mb-5" style={{ color: sol.accent }}>
                      {sol.subtitle}
                    </p>
              ...
              <div className="max-w-7xl mx-auto px-6 sm:px-10 py-16">
                ...
                {"processes" in sol && (
                  <div className="flex flex-col gap-12">
                    {/* 공정 관리 6각형 그리드 */}
                    <div>
                      <SectionLabel accent={sol.accent}>6대 공정 관리</SectionLabel>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
                        {(sol as typeof solutions[0]).processes.map((p) => {
                          const PIcon = p.icon;
                          return (
                            <div key={p.label} className="rounded-2xl p-5 border border-white/6 hover:border-white/12 transition-colors"
                              ...
                              <div className="flex items-center gap-3 mb-3">
                                <div className="w-8 h-8 rounded-lg flex items-center justify-center"
                                  style={{ background: `${sol.accent}20` }}>
                                  <PIcon size={15} style={{ color: sol.accent }} />
                                </div>
                              ...
                              <ul className="flex flex-col gap-1.5">
                                {p.items.map((item) => (
                                  <li key={item} className="flex items-start gap-2 text-xs text-white/50">
                                    <ChevronRight size={12} className="mt-0.5 flex-shrink-0" style={{ color: sol.accent }} />
                                    {item}
                                  </li>
                                ))}
                              </ul>
                    ...
                    {/* 자동화 기능 */}
                    <div>
                      <SectionLabel accent={sol.accent}>구축 자동화 시스템을 통한 스마트 공정 구현</SectionLabel>
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-6">
                        {(sol as typeof solutions[0]).automation.map((item) => (
                          <div key={item} className="flex items-center gap-3 px-4 py-3 rounded-xl border border-white/6 text-sm text-white/70"
                            style={{ background: "rgba(255,255,255,0.03)" }}>
                            <Check size={14} className="flex-shrink-0" style={{ color: sol.accent }} />
                            {item}
                          </div>
                        ))}
                      </div>
                ...
                {/* MooN Editor */}
                {"features" in sol && !("accuracySteps" in sol) && (
                  <div>
                    <SectionLabel accent={sol.accent}>주요 기능</SectionLabel>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mt-6">
                      {(sol as typeof solutions[1]).features.map((f: { icon: string; name: string; desc: string; stage?: string }) => (
                        <div key={f.name} className="rounded-2xl p-6 border border-white/6 hover:border-white/12 transition-all hover:-translate-y-0.5 group"
                          style={{ background: "rgba(255,255,255,0.03)" }}>
                          <div className="flex items-start gap-4">
                            <span className="text-3xl flex-shrink-0">{f.icon}</span>
                            <div>
                              <div className="flex items-center gap-2 mb-2">
                                <h3 className="font-bold text-white text-base group-hover:text-white transition-colors">{f.name}</h3>
                                {"stage" in f && (
                                  <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold"
                                    style={{ background: `${sol.accent}20`, color: sol.accent }}>
                                    {f.stage}
                                  </span>
                                )}
                              </div>
                ...
                {/* MooN AI OCR */}
                {"accuracySteps" in sol && (
                  <div className="flex flex-col gap-12">
                    {/* F1 Score 하이라이트 */}
                    <div className="flex flex-col sm:flex-row gap-8 items-start">
                      <div className="flex-shrink-0 flex flex-col items-center justify-center w-44 h-44 rounded-full border-4"
                        style={{ borderColor: `${sol.accent}50`, background: `${sol.accent}10`, boxShadow: `0 0 60px ${sol.accent}30` }}>
                        <p className="text-xs font-bold text-white/40 uppercase tracking-widest mb-1">평균 F1 Score</p>
                        <p className="text-4xl font-black" style={{ color: sol.accent }}>
                          {(sol as typeof solutions[2]).f1Score}
                        </p>
                      </div>
                      <div className="flex-1">
                        <SectionLabel accent={sol.accent}>주요 기능</SectionLabel>
                        <ul className="mt-4 flex flex-col gap-2.5">
                          {(sol as typeof solutions[2]).features.map((f: string) => (
                            <li key={f} className="flex items-start gap-2.5 text-sm text-white/60">
                              <Check size={14} className="mt-0.5 flex-shrink-0" style={{ color: sol.accent }} />
                              {f}
                            </li>
                          ))}
                        </ul>
                    ...
                    {/* 인식률 향상 그래프 */}
                    <div>
                      <SectionLabel accent={sol.accent}>3차 재학습을 통한 인식률 상승 (고문헌 기준)</SectionLabel>
                      <div className="mt-6 flex flex-col gap-4">
                        {(sol as typeof solutions[2]).accuracySteps.map((step: { label: string; value: string; width: string; highlight?: boolean }) => (
                          <div key={step.label} className="flex items-center gap-4">
                            ...
                            <div className="flex-1 h-9 rounded-xl overflow-hidden" style={{ background: "rgba(255,255,255,0.05)" }}>
                              <div className="h-full rounded-xl flex items-center px-4 transition-all duration-1000"
                                style={{
                                  width: `${step.width}%`,
                                  background: step.highlight
                                    ? `linear-gradient(90deg, ${sol.accent}80, ${sol.accent})`
                                    : `${sol.accent}40`,
                                }}>
                                <span className={`text-sm font-black ${step.highlight ? "text-white" : "text-white/60"}`}>
                                  {step.value}
                                </span>
                              </div>
                            </div>
                ...
                {/* Tango XML */}
                {"features" in sol && "tag" in sol && sol.tag === "emerald" && (
                  <div>
                    <SectionLabel accent={sol.accent}>핵심 기능</SectionLabel>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mt-6">
                      {(sol as typeof solutions[3]).features.map((f: { icon: string; name: string; desc: string }) => (
                        <div key={f.name} className="rounded-2xl p-6 border border-white/6 hover:border-white/15 transition-all hover:-translate-y-1 group"
                          style={{ background: "rgba(255,255,255,0.03)" }}>
                          <span className="text-4xl block mb-4">{f.icon}</span>
                          <h3 className="font-bold text-white mb-2 group-hover:text-white/90">{f.name}</h3>
                ...
                {"outputs" in sol && (
                  <div>
                    <SectionLabel accent={sol.accent}>출력물 유형</SectionLabel>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mt-6">
                      {(sol as typeof solutions[4]).outputs.map((o: { icon: string; name: string; desc: string }, i: number) => (
                        <div key={o.name}
                          className="relative rounded-2xl p-6 border border-white/6 hover:border-white/15 hover:-translate-y-1 transition-all group overflow-hidden"
                          style={{ background: "rgba(255,255,255,0.03)" }}>
                          <div className="absolute top-3 right-4 text-5xl opacity-10 group-hover:opacity-20 transition-opacity">
                ...
                {/* Tango Articlo */}
                {"features" in sol && sol.tag === "rose" && (
                  <div>
                    <SectionLabel accent={sol.accent}>주요 기능</SectionLabel>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mt-6">
                      {(sol as typeof solutions[5]).features.map((f: { icon: string; name: string; desc: string }) => (
                ...
                {"capabilities" in sol && (
                  <div>
                    <SectionLabel accent={sol.accent}>AI 핵심 역량</SectionLabel>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mt-6">
                      {(sol as typeof solutions[6]).capabilities.map((c: { icon: string; name: string; desc: string }) => (
                        <div key={c.name}
                          className="rounded-2xl p-6 border border-white/6 hover:border-white/15 hover:-translate-y-1 transition-all group"
                          style={{ background: "rgba(255,255,255,0.03)" }}>
                          <span className="text-4xl block mb-4">{c.icon}</span>
                          <h3 className="font-bold text-white mb-2 group-hover:text-white/90">{c.name}</h3>
